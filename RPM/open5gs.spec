...
# systemd creates %{_localstatedir}/lib/open5gs-webui (0700, owned by open5gs)
# on start. The app stores its auto-generated session/JWT secrets there via
# OPEN5GS_WEBUI_ENV_FILE, so nothing is ever written to the read-only install.
StateDirectory=open5gs-webui
StateDirectoryMode=0700
Environment=OPEN5GS_WEBUI_ENV_FILE=%{_localstatedir}/lib/open5gs-webui/.env
...
# User-editable configuration. Session/JWT secrets are auto-generated by the
# app into the systemd StateDirectory on first start (see the unit above).
...
- webui: fix open5gs-webui.service crash on start (EACCES writing
  ...
  into an EnvironmentFile; instead the app writes its auto-generated session/
  JWT secrets to a systemd StateDirectory (/var/lib/open5gs/webui/.env, 0700,
  owned by open5gs) via the new OPEN5GS_WEBUI_ENV_FILE override. This removes
