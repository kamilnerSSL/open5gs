...
ogs_pkbuf_t *ogs_pfcp_cp_build_association_release_request(uint8_t type)
{
    ogs_pfcp_message_t *pfcp_message = NULL;
    ogs_pfcp_association_release_request_t *req = NULL;
    ogs_pkbuf_t *pkbuf = NULL;

    ogs_pfcp_node_id_t node_id;
    int node_id_len = 0, rv;

    ogs_debug("Association Release Request");
    ...
    ogs_pfcp_tlv_create_pdr_t *message, int i, ogs_pfcp_pdr_t *pdr)
{
    ...
    if (pdr->f_teid_len) {
        int f_teid_len =
            ogs_min(pdr->f_teid_len, sizeof(pdrbuf[i].f_teid));

        memcpy(&pdrbuf[i].f_teid, &pdr->f_teid, f_teid_len);
        ...
        message->pdi.local_f_teid.len = f_teid_len;
    }
    ...
    ogs_pfcp_tlv_created_pdr_t *message, int i, ogs_pfcp_pdr_t *pdr)
{
    ...
    if (pdr->f_teid_len) {
        ...
        message->local_f_teid.len = f_teid_len;
